...
(function($) {
  ...
  var DEFAULTS = {
    ...
    botWelcomeMessage:
      'jQuery.fbMessenger is a plugin to simulate interaction with a Facebook Messenger bot on an iPhone.',
    leftUser: 'bot',
    rightUser: 'user',
  ...
  var TRANSLATIONS = {
    es: {
      ...
      likesWithFriendAndCount:
        '$LIKES people like this including $FRIENDNAME and $FRIENDCOUNT friends',
      likesWithCount: '$LIKES people like this including $FRIENDCOUNT friends',
      getStartedWarning:
        'When you tap Get Started, $BOTNAME will see your public info.',
      getStartedButton: 'Get Started',
      inputPlaceholder: 'Escribe un mensaje&hellip;'
    },
    en: {
      navBack: 'Home',
      navOptions: 'Manage',
    ...
    de: {
      ...
      likesWithFriendAndCount:
        '$LIKES Personen und $FRIENDNAME und $FRIENDCOUNT weiteren Freunden gefällt das',
      likesWithCount:
        '$LIKES Personen und $FRIENDCOUNT weiteren Freunden gefällt das',
      getStartedWarning:
        "Wenn du auf „Los geht's” tippst, sieht $BOTNAME deine öffentlichen Informationen.",
    ...
    tr: {
      ...
      likesWithFriendAndCount:
        '$FRIENDNAME ve $FRIENDCOUNT arkadaşin dahil $LIKES kişi bunu beğendi',
      likesWithCount: '$FRIENDCOUNT arkadaşin dahil $LIKES kişi bunu beğendi',
      getStartedWarning:
        "Başla'e dokunduğunda, $BOTNAME senin herkese açık bilgilerini görecektir",
    ...
    dk: {
      ...
      likesWithFriend:
        '$LIKES personer synes godt om dette, herunder $FRIENDNAME',
      likesWithFriendAndCount:
        '$LIKES personer synes godt om dette, herunder $FRIENDNAME og $FRIENDCOUNT andre venner',
      likesWithCount:
        '$LIKES personer synes godt om dette, herunder $FRIENDCOUNT venner',
      getStartedWarning:
        'Når du trykker på Kom i gang, kan $BOTNAME se dine offentlige oplysninger.',
  ...
  Plugin.prototype._likeCount = function() {
    var totalCount = this.options.likes
      ? this.options.likes.totalCount
      : undefined;
    var text;
    ...
    if (typeof this.options.likeTextFn === 'function') {
      ...
      if (!short && this.options.likes) {
        if (
          ...
          this.options.likes.otherFriendsCount
        ) {
  ...
  Plugin.prototype._localize = function(key) {
    ...
    if (text.indexOf('$') > -1) {
      ...
      text = text.replace(
        /\$FRIENDNAME/g,
        this.options.likes && this.options.likes.friendName
          ? this.options.likes.friendName
          : ''
      );
      text = text.replace(
        /\$FRIENDCOUNT/g,
        this.options.likes && this.options.likes.otherFriendsCount
          ? this.options.likes.otherFriendsCount
          : '0'
      );
    ...
    var html =
				...
				<div class="jsm-status-navbar">\
					<div class="jsm-status-bar">\
						<div class="jsm-carrier">\
							...
							<span class="jsm-carrier-ring open"></span>\
							<span class="jsm-carrier-name">' +
      this.options.displayedCarrier +
      '</span>\
						</div>\
      ...
      '</div>\
					...
					<div class="jsm-nav-left">\
						...
						<span data-jsm-loc="navBack">' +
      this._localize('navBack') +
      '</span>\
					</div>\
					<div class="jsm-nav-title">\
						<div class="jsm-nav-title-bot-name">' +
      this.options.botName +
      '</div>\
						<div class="jsm-nav-title-replies-in" data-jsm-loc="responseTime">' +
      this._localize('responseTime') +
      '</div>\
					</div>\
					<div class="jsm-nav-right">\
						<span data-jsm-loc="navOptions">' +
      this._localize('navOptions') +
      '</span>\
					</div>\
				</div>\
				...
				<div class="jsm-chat-content">\
					<div class="jsm-chat-progress-indicator jsm-hide">\
						<div class="jsm-bot-icon">\
							<img src="' +
      this.options.botLogoUrl +
      '" />\
						</div>\
					</div>\
					<div class="jsm-bot-welcome-message">\
						<div class="jsm-bot-welcome-banner" style="background-image:url(' +
      this.options.botBannerUrl +
      ')">\
							<div class="jsm-bot-welcome-icon"><img src="' +
      this.options.botLogoUrl +
      '" /></div>\
						</div>\
						<h1>' +
      ...
      '</h1>\
						<h2>' +
      this.options.botCategory +
      '</h2>\
						<p data-jsm-loc="likeTextLong">' +
      this._likeText(false) +
      '</p>\
      ...
      '</p>\
						...
						<div class="jsm-bot-welcome-status">\
							...
							<p>' +
      this.options.botWelcomeMessage +
      '</p>\
						</div>\
					</div>\
      ...
      this.options.botLogoUrl +
      '">\
						<div class="jsm-bot-info-name">' +
      this.options.botName +
      '</div>\
						<div class="jsm-bot-info-likes" data-jsm-loc="likeTextShort">' +
      this._likeText(true) +
      '</div>\
						<div class="jsm-bot-info-category">' +
      this.options.botCategory +
      '</div>\
					</div>\
				</div>\
				...
				<div class="jsm-bottom-bar">\
					...
					<div class="jsm-get-started">\
						<p class="jsm-get-started-info" data-jsm-loc="getStartedWarning">' +
      this._localize('getStartedWarning') +
      '</p>\
						<div class="jsm-get-started-button" data-jsm-loc="getStartedButton">' +
      this._localize('getStartedButton') +
      '</div>\
					</div>\
						...
						<div class="jsm-persistent-menu">\
							<div class="jsm-persistent-menu-scroller">\
								<div class="jsm-persistent-menu-page">\
									<div class="jsm-input-prompt" data-jsm-loc="inputPlaceholder">' +
      this._localize('inputPlaceholder') +
      '</div>';
    if ($.isArray(this.options.persistentMenu)) {
      for (
        var i = 0, max = Math.min(3, this.options.persistentMenu.length);
        i < max;
        ++i
      ) {
        ...
        if (item.label) {
          html +=
            '<div class="jsm-persistent-menu-entry' +
            ($.isArray(item.children) ? ' jsm-has-children' : '') +
            '"><div class="jsm-persistent-menu-text">' +
            item.label +
            '</div></div>';
        }
      }
  ...
  Plugin.prototype.start = function(options) {
    if (options === undefined || options.delay === undefined) {
      if (this.options.state.welcomeMessageDisplayed) {
        this.$element
          .find('.jsm-bot-welcome-message,.jsm-get-started')
          .addClass('jsm-hide');
  ...
  Plugin.prototype._scrollDown = function() {
    var scrollHeight = this.$element
      .find('.jsm-chat-content')
      .prop('scrollHeight');
  ...
  Plugin.prototype._handleResize = function() {
    ...
    setTimeout(function() {
      ...
      var $leftUserWrappers = that.$element.find(
        '.jsm-user-wrapper.' + that.options.leftUser
      );
  ...
  Plugin.prototype._addNewContent = function(user, $payload, timestamp) {
    ...
    if (newWrapper) {
      ...
      if (user === this.options.leftUser) {
        $icon = $(
          '<div class="jsm-user-icon"><img class="jsm-bot-logo" src="' +
            this.options.botLogoUrl +
  ...
  Plugin.prototype.message = function(user, text, options) {
    if (options === undefined || options.delay === undefined) {
      ...
      var sideClass =
        user === this.options.leftUser
          ? 'jsm-left'
          : user === this.options.rightUser
            ? 'jsm-right'
            : '';
      var statusDisplay = '';
  ...
  Plugin.prototype.typingIndicator = function(options) {
    if (options === undefined || options.delay === undefined) {
      this._checkWelcomeMessage();
      this._addNewContent(
        this.options.leftUser,
        $(
          '<div class="jsm-chat-message jsm-left jsm-typing-indicator"><span></span><span></span><span></span></div>'
        ),
        false
      );
    } else {
  ...
  Plugin.prototype.showQuickReplies = function(quickReplies, options) {
    if (options === undefined || options.delay === undefined) {
      ...
      $.each(quickReplies, function(index, quickReply) {
        $quickReplies.append(
          '<div class="jsm-quick-reply-option" style="transition-delay: ' +
            (index * 0.1 * timeScale).toFixed(1) +
            's">' +
            quickReply +
            '</div>'
        );
      ...
      $element
        .find('.jsm-quick-replies')
        .removeClass('jsm-hide')
        .prop('scrollLeft', 0);
  ...
  Plugin.prototype.scrollQuickReplies = function(quickReplyIndex, options) {
    if (options === undefined || options.delay === undefined) {
      ...
      var $option = this.$element.find(
        '.jsm-quick-reply-option:nth-child(' + (quickReplyIndex + 1) + ')'
      );
  ...
  Plugin.prototype.selectQuickReply = function(quickReplyIndex, options) {
    if (options === undefined || options.delay === undefined) {
      ...
      var $option = $element
        ...
        .addClass('jsm-selected');
      var timeScale = this.options.timeScale;
      setTimeout(function() {
        $element.find('.jsm-quick-reply-option').removeClass('jsm-show');
        setTimeout(function() {
          that.message(that.options.rightUser, $option.text(), {
            timestamp: false,
  ...
  Plugin.prototype.showButtonTemplate = function(text, buttons, options) {
    if (options === undefined || options.delay === undefined) {
      this._checkWelcomeMessage();
      var template =
        '<div class="jsm-chat-message jsm-left jsm-button-template"><div class="jsm-header">' +
        text +
        '</div><div class="jsm-buttons">';
  ...
  Plugin.prototype.selectButtonTemplate = function(buttonIndex, options) {
    if (options === undefined || options.delay === undefined) {
      ...
      var $button = this.$element
        .find(
          '.jsm-chat-content .jsm-button-template:last .jsm-buttons .jsm-button:nth-child(' +
            (buttonIndex + 1) +
            ')'
        )
        .addClass('jsm-selected');
      ...
      this.message(this.options.rightUser, $button.text(), {
        timestamp: false
      });
    } else {
      this.options.script.push({
  ...
  Plugin.prototype.showGenericTemplate = function(items, options) {
    if (options === undefined || options.delay === undefined) {
      this._checkWelcomeMessage();
      var template =
        '<div class="jsm-chat-message jsm-left jsm-generic-template-wrapper"><div class="jsm-generic-template-background">';
      var imageClass =
        'jsm-image ' + (options && options.imageSquare ? 'jsm-square' : '');
      $.each(items, function(index, item) {
        template +=
          '<div class="jsm-generic-template ' +
          (index === 0 ? 'jsm-selected' : '') +
          '">';
        if (item.imageUrl) {
          template +=
            '<div class="' +
      ...
      var $templates = this.$element.find(
        '.jsm-generic-template-wrapper:last .jsm-generic-template'
      );
  ...
  Plugin.prototype.scrollGenericTemplate = function(itemIndex, options) {
    if (options === undefined || options.delay === undefined) {
      ...
      var width =
        $scroller.find('.jsm-generic-template:first').outerWidth(true) + 2;
      $scroller.find('.jsm-generic-template').removeClass('jsm-selected');
      $scroller
        .find('.jsm-generic-template:nth-child(' + (itemIndex + 1) + ')')
        .addClass('jsm-selected');
      $scroller.animate(
        {
  ...
  Plugin.prototype.selectGenericTemplate = function(buttonIndex, options) {
    if (options === undefined || options.delay === undefined) {
      ...
      var $button = this.$element
        .find(
          '.jsm-generic-template-wrapper:last .jsm-generic-template.jsm-selected .jsm-button:eq(' +
            buttonIndex +
            ')'
        )
        .addClass('jsm-selected');
      ...
      var sideClass =
        user === this.options.leftUser
          ...
          : user === this.options.rightUser
            ...
            : '';
      var $content = $(
        '<div class="jsm-chat-message jsm-image-attachment ' +
          sideClass +
          ' ' +
          (options.className || '') +
          '"><div class="jsm-image" style="background-image: url(\'' +
          imageUrl +
          '\')"></div>' +
  ...
  Plugin.prototype.annotate = function(annotation, options, index) {
    if (options === undefined || options.delay === undefined) {
      ...
      var $annotation = $(
        '<div class="jsm-annotation ' +
          this.options.annotationClass +
          ' ' +
          ($last.hasClass('jsm-left')
            ? 'jsm-annotation-left'
            : 'jsm-annotation-right') +
          '">' +
          annotation +
          '</div>'
      );
      this.$element
        .find('.jsm-annotation-container')
        .empty()
        .append($annotation);
  ...
  Plugin.prototype.selectPersistentMenu = function(menuItem, options) {
    if (options === undefined || options.delay === undefined) {
      ...
      $container
        .find('.jsm-persistent-menu-page')
        .removeClass('jsm-has-overlay');
      var direction = 0,
        $page;
      if (currentLevel > 0 && menuItem === -1) {
        direction = -1;
        $page = $container.find(
          '.jsm-persistent-menu-page:nth-child(' + currentLevel + ')'
        );
        ...
        var $menuItem = $container
          .find(
            '.jsm-persistent-menu-page:nth-child(' +
              (currentLevel + 1) +
              ') .jsm-persistent-menu-entry:eq(' +
              menuItem +
              ')'
          )
          .addClass('jsm-selected');
        if ($.isArray(currentMenu[menuItem].children) && currentLevel < 2) {
          ...
          $page = $container
            .find(
              '.jsm-persistent-menu-page:nth-child(' + (currentLevel + 2) + ')'
            )
            .empty();
          $page.append(
            '<div class="jsm-persistent-menu-title">' +
              currentMenu[menuItem].label +
              '</div>'
          );
          for (
            var i = 0, max = Math.min(5, currentMenu[menuItem].children.length);
            i < max;
            ++i
          ) {
            $page.append(
              '<div class="jsm-persistent-menu-entry"><div class="jsm-persistent-menu-text">' +
                currentMenu[menuItem].children[i].label +
                '</div></div>'
            );
          }
          level.push(menuItem);
          ...
          setTimeout(function() {
            $container
              .find('.jsm-persistent-menu-entry')
              .removeClass('jsm-selected');
          }, 500 * this.options.timeScale);
          this.message(this.options.rightUser, currentMenu[menuItem].label, {
            timestamp: false
          });
          if (currentLevel > 0) {
  ...
  Plugin.prototype.reset = function() {
    ...
    this.$element
      .find(
        '.jsm-chat-content > :not(".jsm-bot-welcome-message,.jsm-bot-info,.jsm-chat-progress-indicator")'
      )
      .remove();
    this.$element
      .find('.jsm-bot-welcome-message,.jsm-get-started')
      .removeClass('jsm-hide');
    this.$element
      .find('.jsm-input-message,.jsm-chat-progress-indicator')
      .addClass('jsm-hide');
